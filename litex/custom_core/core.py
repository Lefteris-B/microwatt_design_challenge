...
# Microwatt CPU core for LiteX. 
#dir $LITEX_INSTALL_DIR/litex/litex/soc/cores/cpu/microwatt
...
class XICSSlave(LiteXModule):
    ...
    def add_sources(self, platform, use_ghdl_yosys_plugin=False):
        ...
        sdir = get_data_mod("cpu", "microwatt").data_location
        cdir = os.path.dirname(__file__)
        wrapper_path = os.path.join(os.path.dirname(__file__), "xics_wrapper.vhdl")
        self.ics_vhd2v_converter.add_sources(sdir, *sources)
        self.ics_vhd2v_converter.add_source(wrapper_path)
        self.icp_vhd2v_converter.add_sources(sdir, *sources)
        self.icp_vhd2v_converter.add_source(wrapper_path)
