...
# Date: 2023/06/20
# Modified: Added SRAM wrapper internal timing constraints
...
#------------------------------------------#
# SRAM Wrapper Internal Register Constraints
#------------------------------------------#
# The SRAM wrapper gate-level netlist exposes internal state machine 
# registers (_45_, etc.) that aren't characterized in the .lib file.
# These paths from internal registers to external logic cause hold
# violations due to clock skew (~3.9ns).
#
# Option A: Set as false path (if these are async status registers)
# Option B: Set multicycle path (if timing is relaxed)
# Option C: Set minimum delay constraint (forces buffer insertion)

puts "\[INFO\]: Adding SRAM wrapper internal register constraints"

# OPTION A: False path - Use if the internal state registers don't need
# to be timing-clean with external logic (async handshake)
# Uncomment the following if wbs_ack_o path doesn't need single-cycle timing:
# set_false_path -from [get_cells -hierarchical *CF_SRAM_1024x32_wb_wrapper/_45_*]

# OPTION B: Multicycle path - Use if SRAM state machine outputs are
# sampled over multiple cycles (common for memory controllers)
# The _45_ register appears to be the ack/state register
set_multicycle_path -setup 2 -from [get_cells -hierarchical -filter {full_name =~ "*CF_SRAM_1024x32_wb_wrapper/_4?_*"}]
set_multicycle_path -hold 1 -from [get_cells -hierarchical -filter {full_name =~ "*CF_SRAM_1024x32_wb_wrapper/_4?_*"}]
puts "\[INFO\]: Set multicycle path for SRAM wrapper internal registers"

# OPTION C: If you need single-cycle timing but with hold fixing,
# uncomment this to force minimum delay on SRAM output paths:
# set_min_delay 0.5 -from [get_cells -hierarchical *CF_SRAM_1024x32_wb_wrapper/_45_*]
